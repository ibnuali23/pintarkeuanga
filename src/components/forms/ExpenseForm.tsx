...
export function ExpenseForm({ onSubmit, paymentMethods = [] }: ExpenseFormProps) {
  ...
  const { getExpenseSubcategories, categoryNames, categoryIcons } = useDynamicCategories();
  ...
  return (
    <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-4 md:space-y-6 w-full min-w-0">
      <div className="space-y-2">
        ...
        <Popover>
          ...
          <PopoverContent className="w-auto p-0" align="start">
            <Calendar mode="single" selected={selectedDate} onSelect={(date) => date && setValue('date', date)} initialFocus />
          </PopoverContent>
        </Popover>
      ...
      <div className="space-y-2">
        ...
        <Select value={selectedSubcategory} onValueChange={(value) => setValue('subcategory', value)} disabled={!selectedCategory}>
          <SelectTrigger><SelectValue placeholder={selectedCategory ? 'Pilih subkategori' : 'Pilih kategori dulu'} /></SelectTrigger>
          <SelectContent>
      ...
      <div className="space-y-2">
        ...
        <Textarea {...register('description')} placeholder="Contoh: Belanja bulanan di supermarket" className="resize-none" rows={2} />
      ...
      <div className="space-y-2">
        ...
        <div className="relative">
          ...
          <Input type="text" inputMode="numeric" value={amountDisplay} onChange={handleAmountChange} placeholder="0" className="pl-10" />
        </div>
      ...
      {paymentMethods.length > 0 && (
        <div className="space-y-2">
          ...
          <Select value={selectedPaymentMethod} onValueChange={(value) => setValue('payment_method_id', value)}>
              ...
              {paymentMethods.map((method) => (
                <SelectItem key={method.id} value={method.id}>
                  <span className="flex items-center gap-2">
                    ...
                    <span className="text-muted-foreground">({formatCurrency(Number(method.balance))})</span>
                  </span>
                </SelectItem>
              ))}
            </SelectContent>
      ...
      <Button type="submit" disabled={isSubmitting} className="w-full bg-destructive text-destructive-foreground hover:bg-destructive/90" size="lg">
        {isSubmitting ? (
          <Loader2 className="h-5 w-5 animate-spin" />
        ) : (
          <><TrendingDown className="mr-2 h-5 w-5" /> Simpan Pengeluaran</>
        )}
      </Button>
    </form>
  );
