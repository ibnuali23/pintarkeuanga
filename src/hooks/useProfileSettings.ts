...
export function useProfileSettings() {
  ...
  const upsertBudgetSetting = useCallback(async (category: string, subcategory: string, monthlyBudget: number, month: string) => {
    ...
    const { data, error } = await supabase
      ...
      .upsert({
        ...
        month,
      }, { onConflict: 'user_id,category,subcategory,month' })
      .select()
      .single();
    ...
    setBudgetSettings((prev) => {
      const existing = prev.find(b => b.category === category && b.subcategory === subcategory && b.month === month);
      if (existing) {
        return prev.map(b => (b.category === category && b.subcategory === subcategory && b.month === month) ? data : b);
      }
      return [...prev, data];
    });
    return { data };
